...
func NewBerTLV(tag BerTag, value []byte) (*BerTLV, error) {
	...
	for index := 0; index < len(value); {
		...
		if err != nil {
			return nil, fmt.Errorf("tag %02X invalid content: %w", tag, err)
		}
...
func ParseTLV(b []byte) (BerTLVs, error) {
	if len(b) == 0 {
		return nil, errors.New("TLV has length 0")
	}
	...
	for index := 0; index < len(b); {
		...
		if err != nil {
			return BerTLVs{}, fmt.Errorf("invalid TLV starting at index %d", index)
		}
...
func parseFirstBerTLV(b []byte) (berTLV BerTLV, lenParsed int, err error) {
	tag, err := parseTag(b)
	if err != nil {
		return BerTLV{}, 0, fmt.Errorf("invalid tag at start: %02X", b)
	}
	...
	if err != nil {
		return BerTLV{}, 0, fmt.Errorf("tag %02X: invalid length encoding", tag)
	}
	...
	if endIndex := len(b) - 1; indicatedEndIndex > endIndex {
		return BerTLV{}, 0, fmt.Errorf("tag %02X: indicated length of value is out of bounds - indicated end index: %d actual end index %d", tag, indicatedEndIndex, endIndex)
	}
	...
	if tag.IsConstructed() {
		...
		for valueIndex := 0; valueIndex < len(value); {
			...
			if err != nil {
				return BerTLV{}, 0, fmt.Errorf("tag %02X: invalid child object: %w", tag, err)
			}
...
func (t BerTag) CheckEncoding() error {
	...
	if l > 3 {
		return fmt.Errorf("tags must consist of a maximum of three bytes, got %d", len(t))
	}
	...
	if t[0]&0x1F != 0x1F {
		return fmt.Errorf("tag consists of %d byte but first byte does not indicate that more bytes follow", len(t))
	}
